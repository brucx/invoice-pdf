...
We later isolated the root causes: `@pdf-lib/fontkit` is a stale fork predating two upstream fixes — for TrueType, the subset `loca` is written in short format without padding glyf records to even length, so one odd-length glyph shifts every later offset by a byte ([fixed upstream](https://github.com/foliojs/fontkit/pull/191) in fontkit 1.8.0); for CID-keyed CFF, the CFF header's `offSize` is written as the source table length truncated to a byte (27 — spec says 1–4), making renderers reject the whole font, plus FDSelect misattribution ([fixed upstream](https://github.com/foliojs/fontkit/pull/230) in 1.9.0). Documented with repro in [pdf-lib#1232](https://github.com/Hopding/pdf-lib/issues/1232#issuecomment-5057738934) and reported to the active fork in [cantoo-scribe/pdf-lib#144](https://github.com/cantoo-scribe/pdf-lib/issues/144). By the time we knew all this, the HarfBuzz route below had already shipped — and it produces smaller PDFs anyway (per-document subsets beat per-charset subsets).
